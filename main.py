...
class Login(Frame):
    ...
    def process4(self,r):  # Morphology: fill holes
        ...
        cv2.imwrite('test/p4'+r+'.png', gi_out)

    def process5(self,r):  # Morphing To eliminate small objects
        img = cv2.imread('test/p4'+r+'.png')
        kernel = np.ones((5, 5), np.uint8)
        open = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel)
        close = cv2.morphologyEx(open, cv2.MORPH_CLOSE, kernel)
        cv2.imwrite('test/p5'+r+'.png', close)

    def process7(self,r):  #Histogram
        img = cv2.imread('test/p5'+r+'.png', 0)
        img2 = cv2.imread('test/p1'+r+'.png', 0)
